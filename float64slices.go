...
func CoordsToFloat64Slices(coords []Coord) [][]float64 {
	...
	float64Slices := make([][]float64, 0, len(coords))
	for i := range coords {
		float64Slice := coords[i][:]
		float64Slices = append(float64Slices, float64Slice)
	}
...
func Float64SlicesToCoords(float64Slices [][]float64) []Coord {
	...
	coords := make([]Coord, 0, len(float64Slices))
	for _, float64Slice := range float64Slices {
		var coord Coord
